<template>
    <div id="mailing-app" class="border border-solid d-theme-border-grey-light rounded relative overflow-hidden">

        <vs-sidebar class="items-no-padding" parent="#mailing-app" :click-not-close="clickNotClose" :hidden-background="clickNotClose" v-model="isMailingSidebarActive">
            <mailing-sidebar :mailingCats="mailingCats" @close-sidebar="toggleMailingSidebar" :mailingFilter="mailingFilter" />
        </vs-sidebar>
        ...
        <div :class="{'sidebar-spacer': clickNotClose}" class="no-scroll-content border border-solid d-theme-border-grey-light border-r-0 border-t-0 border-b-0">
            ...
            <div class="flex border d-theme-dark-bg items-center">
                <feather-icon class="md:inline-flex lg:hidden ml-4 mr-4 cursor-pointer" icon="MenuIcon" @click.stop="toggleMailingSidebar(true)"/>
                <vs-input icon-no-border icon="icon-search" size="large" icon-pack="feather" placeholder="Поиск рассылки" v-model="searchQuery" class="vs-input-no-border vs-input-no-shdow-focus w-full" />
            </div>

            <!-- EMAILS LIST -->
            <component :is="scrollbarTag" class="mailing-content-scroll-area" :settings="settings" ref="mailListPS">
              <div>
                <transition-group name="list-enter-up" class="mailing__mailings" tag="ul" appear v-if="mailings && mailings.length">
                    <li class="cursor-pointer wmailing__mailing-item" v-for="(mailing, index) in mailings" :key="String(mailingFilter) + String(mailing.id)" @click="updateOpenMailing(mailing.id)" :style="{transitionDelay: (index * 0.1) + 's'}">
                        <mailing-item :mailing="mailing" :isSelected="isMailSelected(mailing.id)" @addToSelected="addToSelectedMails" @removeSelected="removeSelectedMail" />
                    </li>
                </transition-group>
                <div v-else>
                  <vx-card style="text-align: center; margin-top: 20px;">
                    Таких рассылок нет!, Нажмите Создать рассылку в левом углу.
                  </vx-card>
                </div>
              </div>
            </component>
        ...
        <!-- EMAIL VIEW SIDEBAR -->
        <mailing-view v-if="openMailingId"
...
export default {
  ...
  watch: {
    ...
    mailingFilter () {
      ...
      this.fetchMailings({ filter: `status||$eq||${this.getStatusValue(this.$route.params.filter)}` }); // Fetch Mailings
  ...
  computed: {
    ...mapState('mailing', ['mailingCats', 'mailingSearchQuery', 'mailings']),
  ...
  methods: {
    ...
    ...mapActions('mailing', ['setMailingSearchQuery', 'fetchMailingCats', 'fetchMailings']),
    ...
    updateLabels (label) {
      this.$store.dispatch('mailing/updateLabels', { mails: this.selectedMails, label })
      this.selectedMails = []
    },
